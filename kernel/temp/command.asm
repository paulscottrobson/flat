...
WarmStartSetup:
	...
	ld   hl,(SIScreenSize)      ; clear the 2nd line
	ld   de,-64
	add  hl,de
	ld   b,32
__CLIClear:
	...
	ld   hl,(SIScreenSize)      ; half way down 2nd line.
	ld   de,-48
	add  hl,de
...
__CLIPromptExit:
	...
	ld   hl,(SIScreenSize)     ; bakck to start of 2nd to last row
	ld  de,-64
	add  hl,de
...
__CLIExecute:
	...
	ld   ix,__CLIBuffer-1      ; get address of buffer into BC
	ld   a,l         ; get length
	and  $0F
	or   $80         ; 1 10 lllll
	ld   (ix+0),a        ; make that the tag/

	ld   hl,(__ARegister)      ; Execute as a yellow word in context.
	ld   de,(__BRegister)
	call  PROProcessTaggedWord
	ld   (__BRegister),de
	ld   (__ARegister),hl
	jp   WarmStart
